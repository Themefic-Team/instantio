    ...
    $(document).ready(function () {
        ...
        $(window).on("hashchange load", function () {
            ...
            if (query.indexOf("dashboard") > -1) {
                let submenu = $("#toplevel_page_" + instantio_options.option_id).find(
                    ".wp-submenu"
                );
                submenu
                    .find("a")
        ...
        $(document).on("click", ".tf-tablinks", function (e) {
            ...
            if ($this.next().hasClass("tf-submenu")) {
                firstTabId = parentDiv
                    .find(".tf-submenu li:first-child .tf-tablinks")
                    .data("tab");
            }
        ...
        const tfDateInt = (dateSelector) => {
            $(dateSelector).each(function () {
                ...
                if (dateField.length === 2) {
                    let startDate = $this
                        .find(".tf-date-from input.flatpickr")
                        .flatpickr({
                            dateFormat: format,
                        ...
                        $tffields.each(function () {
                            ...
                            $.each(controllers, function (index, depend_id) {
                                ...
                                ruleset = ruleset.createRule(
                                    $this.find('[data-depend-id="' + depend_id + '"]'),
                                    condition,
                                    value
                                );
        ...
        $(document).on(
            "click",
            ".tf-icon-select .tf-admin-btn, .tf-icon-select .tf-icon-preview",
            function (e) {
                e.preventDefault();
                let btn = $(this);
        ...
        $(document).on("submit", ".tf-option-form.tf-ajax-save", function (e) {
            ...
            data.append("action", "instantio_options_save");
        ...
        */
        ...
        $(document).on("click", ".tf-repeater-icon-add", function () {
            ...
            var add_value = $this_parent
                .find(
                    ".tf-single-repeater-clone-" + id + " .tf-single-repeater-" + id + ""
                )
                .clone();
            ...
            add_value.find("[data-depend-id]").each(function () {
                ...
                if (typeof data_depend_id !== "undefined") {
                    data_depend_id = data_depend_id.replace(
                        "[" + current_field + "][00]",
                        "[" + current_field + "][" + count + "]"
                    );
            ...
            add_value.find("[data-controller]").each(function () {
                ...
                if (typeof data_controller !== "undefined") {
                    data_controller = data_controller.replace(
                        "[" + current_field + "][00]",
                        "[" + current_field + "][" + count + "]"
                    );
        ...
        $(document).on("click", ".tf-repeater-icon-clone", function () {
            ...
            clone_value.find(".tf-field-select2").each(function () {
                var get_selected_value = $(this)
                    .find("select.tf-select-two")
                    .select2("val");
                $(this)
                    .find("select.tf-select-two")
                    .removeAttr("data-select2-id aria-hidden tabindex");
                $(this)
                    .find("select.tf-select-two option")
            ...
            // Clone Wp Editor
            clone_value
                .find("textarea.parent_wp_editor, textarea.wp_editor")
                .each(function () {
                    var count = Math.random().toString(36).substring(3, 9) + 1;
                    $(this).attr("id", current_field + count);
                    $(this).attr("data-count-id", count);
            ...
            // Clone Select 2
            clone_value
                .find("select.tf-select2-parent, select.tf-select2")
                .each(function () {
                    this.id = this.id.replace(
                        "" + current_field + "__" + repeater_count,
                        "" + current_field + "__" + count + ""
                    );
                    var $this = $(this);
                    tfSelect2Int($this);
        ...
        $(document).on(
            ...
            function () {
                ...
                $(this)
                    ...
                    .find(".tf-repeater-content-wrap")
                    .slideToggle();
                $(this)
                    .closest(".tf-single-repeater-" + tf_repater_fieldname + "")
                    .children(".tf-repeater-content-wrap")
                    .toggleClass("hide");
                if (
                    $(this)
                        .closest(".tf-single-repeater-" + tf_repater_fieldname + "")
                    ...
                    $(this)
                        .closest(
                            ".tf-single-repeater-" +
                            tf_repater_fieldname +
                            " .tf-repeater-header"
                        )
                        .children(".tf-repeater-icon-collapse")
        ...
        $(".tf-repeater-wrap").each(function () {
            ...
            $($this).sortable({
                ...
                start: function (event, ui) {
                    // turn TinyMCE off while sorting (if not, it won't work when resorted)
        ...
        $(document).on("click", ".tf-tab-item", function () {
            ...
            if (
                $this.parent().parent().find(".tf-tab-item-content").hasClass("show") ==
                true
            ) {
                $this
                    .parent()
                    .parent()
                    .find(".tf-tab-item-content")
                    .removeClass("show");
            }
            ...
            $this
                .parent()
                .parent()
                .find(".tf-tab-item-content[data-tab-id = " + tab_id + "]")
                .addClass("show");
        });
    });
...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $("body").on("click", ".tf-media-upload", function (e) {
            ...
            frame.on("select", function () {
                ...
                $this
                    .parent()
                    .parent()
                    .find(".tf-fieldset-media-preview")
                    .html(
                        `<div class="tf-image-close" tf-field-name='${fieldname}'>✖</div><img src='${attachment.url}' />`
                    );
            });
            frame.open();
            return false;
        ...
        if (instantio_options.gmaps != "googlemap") {
            $(".tf-field-map").each(function () {
                ...
                if (!$search_input.length) {
                    $search_input = $(
                        '[data-depend-id="' +
                        $this.find(".tf--address-field").data("address-field") +
                        '"]'
                    );
                }
...
(function ($) {
    $(document).ready(function () {
        if (instantio_options.tf_chart_enable == 1) {
                    ...
                    datasets: [
                        {
                            ...
                            borderColor: "#003C79",
                            tension: 0.1,
                            data: instantio_options.tf_complete_order,
                            fill: false,
                        },
                        {
                            ...
                            borderColor: "red",
                            tension: 0.1,
                            data: instantio_options.tf_cancel_orders,
                            fill: false,
                        },
                    ],
                },
        ...
        $(document).on("change", "#tf-month-report", function () {
            ...
            if (monthTarget != 0) {
                ...
                jQuery.ajax({
                    ...
                    url: instantio_options.ajax_url,
                    data: {
                        action: "instantio_month_reports",
                        month: monthTarget,
                    },
